function roidb_new = generate_proposal(conf, model_stage, imdb, roidb)
  ...
  roidb_new                   = roidb_from_proposal_ilsvrc(imdb, roidb, roidb_regions,'keep_raw_proposal', false);    
end
...
function regions = make_roidb_regions(aboxes, images)
   ...
   for i = 3:length(images)
    regions.images{i} =images(i).name;
   end
end
